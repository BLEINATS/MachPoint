...
const RentalItemsTab: React.FC = () => {
  ...
  const handleSave = async (itemData: Omit<RentalItem, 'id' | 'arena_id' | 'created_at'> | RentalItem) => {
    ...
    const dataToSave = { ...itemData, arena_id: arena.id };
    if (!isEditing) delete (dataToSave as any).id;

    try {
      const { error } = await supabase.from('rental_items').upsert(dataToSave);
      if (error) throw error;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const { error } = await supabase.from('rental_items').delete().eq('id', itemToDelete.id);
      if (error) throw error;
            ...
            <tbody className="bg-white dark:bg-brand-gray-800 divide-y divide-brand-gray-200 dark:divide-brand-gray-700">
              {items.map(item => (
                <tr key={item.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-brand-gray-500 dark:text-brand-gray-400">{item.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
