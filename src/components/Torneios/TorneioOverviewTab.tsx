...
interface TorneioOverviewTabProps {
  torneio: Torneio;
}
...
const TorneioOverviewTab: React.FC<TorneioOverviewTabProps> = ({ torneio }) => {
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard icon={Users} label="Inscritos" value={`${torneio.participants.length} / ${torneio.max_participants}`} color="text-blue-500" />
        <StatCard icon={DollarSign} label="Taxa" value={torneio.registration_fee.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} color="text-green-500" />
        <StatCard icon={Trophy} label="Tipo" value={getTypeLabel(torneio.type)} color="text-purple-500" />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
            ...
            <div className="flex items-start">
                ...
                <div>
                    ...
                    <p className="font-semibold text-brand-gray-800 dark:text-brand-gray-200">{torneio.start_time} - {torneio.end_time}</p>
                </div>
            </div>
             <div className="flex items-start">
                ...
                <div>
                    ...
                    <p className="font-semibold text-brand-gray-800 dark:text-brand-gray-200">{torneio.quadras_ids.length} quadra(s) reservada(s)</p>
                </div>
            </div>
             <div className="flex items-start">
