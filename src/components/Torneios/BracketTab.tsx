...
const BracketTab: React.FC<BracketTabProps> = ({ torneio, setTorneio }) => {
  ...
  const rounds = Array.from(new Set(torneio.matches.map(m => m.round))).sort((a, b) => a - b);
      ...
      <div className="flex overflow-x-auto space-x-8 pb-4">
        {rounds.map(round => (
          <div key={round} className="flex flex-col space-y-8 min-w-[280px]">
            ...
            {torneio.matches.filter(m => m.round === round).map(match => {
