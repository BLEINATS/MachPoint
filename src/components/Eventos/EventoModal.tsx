...
import { Evento, Quadra, EventoTipoPrivado, EventoStatus, Reserva } from '../../types';
import Button from '../Forms/Button';
import Input from '../Forms/Input';
import { format, parse, isSameDay, isBefore, eachDayOfInterval } from 'date-fns';
...
const EventoModal: React.FC<EventoModalProps> = ({ isOpen, onClose, onSave, initialData, quadras, reservas }) => {
  ...
  useEffect(() => {
      ...
      for (const day of eventDays) {
        for (const quadraId of formData.quadras_ids) {
          const conflictingReserva = allExpandedReservations.find(r => {
            ...
            const eventStartTime = parse(formData.startTime, 'HH:mm', day);
            const eventEndTime = parse(formData.endTime, 'HH:mm', day);
    ...
    checkConflicts();
  }, [formData.quadras_ids, formData.startDate, formData.endDate, formData.startTime, formData.endTime, reservas, quadras, isEditing, initialData?.id, isOpen]);
              ...
              <Section title="Data e Espaços">
                <p className="text-sm -mt-2 text-brand-gray-500 dark:text-brand-gray-400">Selecione data, hora e os espaços necessários para o evento. A reserva de quadras é opcional.</p>
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Input label="Horário Início" name="startTime" type="time" value={formData.startTime} onChange={handleChange} />
                    <Input label="Horário Fim" name="endTime" type="time" value={formData.endTime} onChange={handleChange} />
                 </div>
                 <div>
