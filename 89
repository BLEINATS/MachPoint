...
import { Check, UserPlus, Send, BarChart3, X } from 'lucide-react';
...
const ParticipantsTab: React.FC<ParticipantsTabProps> = ({ torneio, setTorneio }) => {
  ...
  const handleGenerateBracket = () => {
    if (torneio.participants.length < 2) {
      alert("É necessário ter pelo menos 2 participantes para gerar as chaves.");
      return;
    }
    const newMatches = generateBracket(torneio.participants);
    setTorneio(prev => prev ? { ...prev, matches: newMatches } : null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <h3 className="text-xl font-semibold">Lista de Inscritos ({torneio.participants.length})</h3>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-brand-gray-200 dark:divide-brand-gray-700">
            ...
            <tbody className="bg-white dark:bg-brand-gray-800 divide-y divide-brand-gray-200 dark:divide-brand-gray-700">
              {torneio.participants.map((participant, index) => (
                <motion.tr 
                  key={participant.id}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
      ...
      <div className="bg-blue-50 dark:bg-brand-blue-500/10 border-l-4 border-blue-400 p-4 rounded-r-lg">
        <div className="flex items-center justify-between">
            ...
            <Button onClick={handleGenerateBracket} disabled={torneio.matches.length > 0}>
                <BarChart3 className="h-4 w-4 mr-2" />
                {torneio.matches.length > 0 ? 'Chaves Geradas' : 'Gerar Chaves'}
            </Button>
        </div>
      </div>
    </div>
