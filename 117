...
import { Torneio, Quadra, Reserva, Participant } from '../types';
...
const Torneios: React.FC = () => {
  ...
  const loadData = useCallback(() => {
    if (arena) {
      ...
      setTorneios(savedTorneios ? JSON.parse(savedTorneios) : []);
  ...
  const handleSaveTorneio = (torneioData: Omit<Torneio, 'id' | 'arena_id' | 'created_at'> | Torneio) => {
    if (!arena) return;

    const isEditing = 'id' in torneioData;
    const torneioId = isEditing ? torneioData.id : `torneio_${Date.now()}`;
    
    const participants = isEditing ? (torneioData as Torneio).participants : MOCK_PARTICIPANTS.slice(0, torneioData.max_participants).map((p, i) => ({...p, id: `participante_${torneioId}_${i}`}));
    ...
    const updatedTorneios = isEditing
      ? torneios.map(e => e.id === newTorneio.id ? newTorneio : e)
      : [...torneios, newTorneio];
    ...
    for (const day of tournamentDays) {
      for (const quadraId of newTorneio.quadras_ids) {
        ...
        tournamentBlockReservations.push(newBlockReserva);
      }
    }

    const finalReservas = [...otherReservas, ...tournamentBlockReservations];
  ...
  const handleDeleteTorneio = (id: string) => {
    ...
    const updatedTorneios = torneios.filter(e => e.id !== id);
    setTorneios(updatedTorneios);
    localStorage.setItem(`torneios_${arena.id}`, JSON.stringify(updatedTorneios));
