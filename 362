...
-- 2. Recreate ensure_aluno_profile_exists function (for consistency)
/*
...
END;
$$;
-- 3. Recreate create_client_reservation_atomic function with the type cast fix
/*
# [Function] create_client_reservation_atomic
Cria uma reserva de cliente de forma atômica, corrigindo o tipo do status de pagamento.
*/
CREATE OR REPLACE FUNCTION public.create_client_reservation_atomic(
    ...
    p_payment_status text, -- Parameter remains text
    p_sport_type text,
...
BEGIN
    ...
    VALUES (
        ...
        'avulsa',
        p_total_price,
        p_payment_status::public.payment_status, -- FIX: Explicitly cast text to payment_status type
        p_sport_type,
        p_credit_to_use,
        p_rented_items,
        p_client_name,
