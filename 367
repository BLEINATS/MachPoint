/*
  # [Operation Name]
  [This operation provides a definitive fix for the "function is not unique" error by safely dropping all conflicting versions of the reservation functions and recreating them with the correct signatures and logic.]
  ## Query Description: [This script resolves a persistent and complex error by removing several conflicting database functions and recreating them with the correct structure. This is a critical fix to restore the client reservation functionality. The operation is designed to be safe and will not affect existing data, but it modifies core application logic.]
  ## Metadata:
...
DROP FUNCTION IF EXISTS public.check_duplicate_reservation(uuid, date, time, time, uuid);
DROP FUNCTION IF EXISTS public.check_duplicate_reservation(uuid, uuid, date, character varying, character varying);
...
BEGIN
    ...
    IF v_aluno_id IS NOT NULL THEN
        RETURN v_aluno_id;
    END IF;
    -- If not, create a new aluno profile using the provided name and phone
...
-- Step 3: Recreate 'check_duplicate_reservation' with the correct signature.
...
BEGIN
    ...
    -- Handle overnight bookings by treating 00:00 as the end of the day for overlap checks.
...
BEGIN
    -- Ensure an 'aluno' profile exists for this user in this arena
    ...
    -- Add gamification points if the system is active
